...
.controller('DocsController', function($scope, $rootScope, $location, $window, $cookies, SOFA_PAGES, SOFA_NAVIGATION, SOFA_VERSION) {
  ...
  $scope.$watch(function docsPathWatch() {return $location.path(); }, function docsPathWatchAction(path) {
    ...
    var currentPage = $scope.currentPage = SOFA_PAGES[path];
    if ( !currentPage && path.charAt(0)==='/' ) {
      // Strip off leading slash
      path = path.substr(1);
    }

    currentPage = $scope.currentPage = SOFA_PAGES[path];
    ...
    if ( currentPage ) {
      $scope.currentArea = currentPage && SOFA_NAVIGATION[currentPage.area];
      var pathParts = currentPage.path.split('/');
      var breadcrumb = $scope.breadcrumb = [];
      var breadcrumbPath = '';
      angular.forEach(pathParts, function(part) {
        breadcrumbPath += part;
        breadcrumb.push({ name: (SOFA_PAGES[breadcrumbPath]&&SOFA_PAGES[breadcrumbPath].name) || part, url: breadcrumbPath });
        breadcrumbPath += '/';
      });
    } else {
